...
const AegisoraSpark = () => {
  return (
    <div className="w-5 h-5 flex-shrink-0 text-[#0066EE]">
...
export default function Footer() {
  return (
    <footer className="w-full pt-20 pb-12 px-6 bg-transparent font-sans text-slate-600 relative z-10 border-t border-slate-300/40">
      <div className="max-w-[1200px] mx-auto flex flex-col items-center">
        {/* GET STARTED KARTI (Butonlar /login sayfasına yönlendirir) */}
        <div className="w-full max-w-[1100px] bg-white rounded-[2.5rem] shadow-[0_20px_60px_rgba(0,0,0,0.08)] border border-slate-200/80 overflow-hidden grid grid-cols-1 lg:grid-cols-12 min-h-[500px] mb-24">
          {/* Sol Taraf: Kayıt / Giriş Alanı */}
          <div className="lg:col-span-6 p-10 sm:p-14 lg:p-16 flex flex-col justify-center items-start">
            ...
            <div className="w-full space-y-4 max-w-sm">
              {/* Google Continue Butonu -> /login sayfasına yönlendirir */}
              <Link
                href="/login"
              ...
              {/* Email Continue Butonu -> /login sayfasına yönlendirir */}
              <Link
                href="/login"
          ...
          {/* Sağ Taraf: Dalga Dalga Dönen/Akın Eden Premium Animasyonlu Alan */}
          <div className="lg:col-span-6 bg-[#0a0a0a] relative overflow-hidden flex items-center justify-center p-12">
            {/* Arka plan akışkan dalgalar (Wave Gradients) */}
            <div className="absolute inset-0 pointer-events-none opacity-40">
              <motion.div
                ...
                className="absolute -top-20 -right-20 w-[350px] h-[350px] rounded-full bg-gradient-to-br from-blue-600/40 via-indigo-600/30 to-transparent blur-[80px]"
              />
              <motion.div
                animate={{
                  scale: [1.2, 1, 1.2],
                ...
                className="absolute -bottom-20 -left-20 w-[400px] h-[400px] rounded-full bg-gradient-to-tr from-blue-500/35 via-cyan-500/20 to-transparent blur-[90px]"
              />
            </div>

            {/* Dönen / Dalgalanan Geometrik Veri Çemberleri (Loop Animation) */}
            <div className="relative z-10 w-full max-w-[320px] h-[320px] flex items-center justify-center">
              {/* Dış Halka */}
              <motion.div
                animate={{ rotate: 360 }}
                transition={{ duration: 25, repeat: Infinity, ease: "linear" }}
                className="absolute inset-0 border border-blue-500/20 rounded-full flex items-center justify-center"
              >
                <div className="absolute top-0 w-2 h-2 bg-blue-500 rounded-full shadow-[0_0_12px_#3b82f6]"></div>
              </motion.div>

              {/* Orta Halka (Ters Yöne Döner) */}
              <motion.div
                animate={{ rotate: -360 }}
                transition={{ duration: 20, repeat: Infinity, ease: "linear" }}
                className="absolute inset-8 border border-white/10 rounded-full flex items-center justify-center border-dashed"
              >
              ...
              <motion.div
                ...
                className="absolute inset-16 border border-blue-400/30 rounded-full flex items-center justify-center"
              >
                <div className="absolute right-0 w-1.5 h-1.5 bg-cyan-400 rounded-full shadow-[0_0_10px_#22d3ee]"></div>
              </motion.div>

              {/* Merkez Çekirdek (Pulse Efekti) */}
              <motion.div
                ...
                className="w-24 h-24 rounded-2xl bg-gradient-to-br from-blue-600/30 to-indigo-600/30 border border-blue-500/40 backdrop-blur-xl flex flex-col items-center justify-center shadow-[0_0_30px_rgba(0,102,238,0.3)]"
              >
                <svg
