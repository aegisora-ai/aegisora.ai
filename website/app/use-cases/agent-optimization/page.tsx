    ...
    <div className="min-h-screen flex flex-col bg-white font-sans">
      ...
      <div className="max-w-[1000px] mx-auto px-6 py-20 flex-1">
        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-blue-50 border b order-blue-100 text-[#0066FF] text-[12px] font-bold mb-6">
          <Activity className="w-3.5 h-3.5" /> Use Cases
        </div>

        <h1 className="text-[44px] md:text-[56px] font-black text-slate-900 tracking-tight mb-6">Automated Agent Optimization</h1>
        <p className="text-[18px] text-slate-600 font-medium leading-relaxed mb-10">
          Optimize token consumption, latency, and model routing automatically to reduce infrastructure costs by up to 40%.
        </p>
        ...
        <div className="space-y-6">
          {[
            { title: "Smart Model Fallbacks", desc: "Automatically route simple queries to smaller, cheaper models while reserving GPT-4o for complex reasoning." },
            { title: "Semantic Caching", desc: "Cache common agent responses securely in-memory to eliminate redundant API calls and latency." },
            { title: "Token Budget Enforcement", desc: "Set strict cost ceilings per agent, user, or department to prevent runaway LLM bills." }
          ].map((item, idx) => (
