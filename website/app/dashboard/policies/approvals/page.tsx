      ...
      <div className="flex flex-col gap-4">
        {filteredApprovals.length === 0 ? (
          ...
          filteredApprovals.map((req) => (
            <div key={req.id} className="bg-[#111113] border b order-border hover:b order-sidebar-ring rounded-xl p-5 shado w-sm transition-colors flex flex-col md:flex-row gap-6 justify-between">

              <div className="flex-1 flex flex-col gap-3">
                <div className="flex items-center gap-3">
                  <span className="text-[11px] font-mono text-muted-foreground bg-[#18181b] border b order-border px-2 py-0.5 rounded">{req.id}</span>
                  <span className="flex items-center gap-1.5 text-[11px] font-medium text-decision-block bg-decision-block/10 border b order-decision-block/20 px-2 py-0.5 rounded">
              ...
              <div className="flex flex-row md:flex-col items-center md:items-end justify-center gap-3 b order-t md:b order-t-0 md:b order-l b order-border/50 pt-4 md:pt-0 md:pl-6">
                <button className="flex items-center justify-center gap-2 w-full md:w-32 bg-decision-allow/10 hover:bg-decision-allow/20 text-decision-allow border b order-decision-allow/30 px-4 py-2 rounded-lg text-[13px] font-medium transition-colors outline-none cursor-pointer">
                  <Check className="w-4 h-4" /> Approve
                </button>
                <button className="flex items-center justify-center gap-2 w-full md:w-32 bg-decision-block/10 hover:bg-decision-block/20 text-decision-block border b order-decision-block/30 px-4 py-2 rounded-lg text-[13px] font-medium transition-colors outline-none cursor-pointer">
