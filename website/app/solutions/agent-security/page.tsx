    ...
    <div className="min-h-screen bg-white dark:bg-[#0d0d0f] text-slate-900 dark:text-slate-100 font-sans selection:bg-blue-100 dark:selection:bg-blue-900/50 transition-colors duration-300">
      ...
      <header className="flex items-center justify-between px-6 py-4 b order-b b order-slate-200 dark:b order-white/10 bg-white/90 dark:bg-[#0d0d0f]/90 backdrop-blur-md sticky top-0 z-50">
        ...
        <nav className="hidden lg:flex items-center gap-8 text-[14px] font-semibold text-slate-600 dark:text-slate-300">
          ...
          <Link href="#" className="text-blue-600 dark:text-blue-500 transition-colors">Agent Security</Link>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            <div className="max-w-2xl z-10">
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} className="inline-flex items-center gap-2.5 px-3 py-1.5 rounded-lg bg-blue-50 dark:bg-blue-900/20 border b order-blue-200 dark:b order-blue-800/50 text-slate-800 dark:text-blue-100 text-[13px] font-bold tracking-wide uppercase mb-8">
                <div className="w-5 h-5 rounded flex items-center justify-center bg-blue-100 dark:bg-blue-800"><Bot className="w-3 h-3 text-blue-600 dark:text-blue-300" /></div>
                Solutions / Agent Security
              </motion.div>
              <motion.h1 initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: 0.1 }} className="text-[40px] sm:text-[56px] lg:text-[72px] font-bold tracking-[-0.03em] text-slate-900 dark:text-white leading-[1.05] mb-6" dangerouslySetInnerHTML={{ __html: "Govern autonomous agents <br /> and tool access." }} />
              <motion.p initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: 0.2 }} className="text-[18px] md:text-[20px] text-slate-600 dark:text-slate-400 leading-relaxed mb-10">
                Autonomous agents need strict boundaries. Enforce Identity and Access Management (IAM), limit API access, and intercept destructive actions before they execute.
              </motion.p>
            ...
            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6, delay: 0.2 }} className="relative z-10 w-full h-[400px] rounded-2xl bg-white dark:bg-[#111113] border b order-slate-200 dark:b order-white/10 shado w-2xl flex items-center justify-center overflo w-hidden">
               ...
               <Bot className="w-24 h-24 text-blue-500 absolute drop-shado w-2xl" strokeWidth={1} />
            </motion.div>
          </div>
            ...
            <div className="bg-[#f8fafc] dark:bg-[#111113] border b order-slate-200/50 dark:b order-white/10 rounded-3xl p-10 md:p-16 shado w-2xl relative overflo w-hidden transition-colors duration-300">
              <div className="relative z-10">
                <h2 className="text-[36px] md:text-[48px] font-bold text-slate-900 dark:text-white mb-6 leading-[1.1] tracking-tight max-w-2xl">
                  Ready to deploy Agent Security at scale?
                </h2>
